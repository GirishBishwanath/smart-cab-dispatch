...
const initializeSocket = (httpServer) => {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            if (!token) {
                logger.warn("socket.auth.rejected", { reason: "missing_token" });
    ...
    io.on("connection", (socket) => {
        ...
        logger.info("socket.connected", { userId, role: socket.user.role });
        ...
        socket.emit("socket:connected", { connected: true, userId });

        socket.on("driver:location", async (payload) => {
            try {
                if (socket.user.role !== ROLES.DRIVER) return;

                const { rideId, latitude, longitude, clientUpdatedAt } = payload || {};
                ...
                if (clientUpdatedAt != null && !Number.isFinite(clientTimestamp)) return;

                const now = Date.now();

                if (clientTimestamp != null && clientTimestamp > now + 30_000) return;
...
export { initializeSocket, getIO, closeSocket, emitToUser };
