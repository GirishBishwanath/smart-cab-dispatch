  ...
  if (!ride) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-sm font-bold text-sky-600">
          Go back
        </button>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Panel icon={FaRoute} title="Ride information">
          <Info title="Trip type" value={ride.tripType} />
        ...
        <Panel icon={FaCar} title="Driver information">
          ...
          <Info title="Vehicle" value={ride.driver?.vehicle?.vehicleNumber} />
        </Panel>
        ...
        <Panel icon={FaRoute} title="Route information">
          ...
          <Info
            title="Completed"
            value={ride.status === "COMPLETED" ? formatDateTime(ride.completedAt) : "—"}
          />
        </Panel>
      </div>
