...
const Hero = () => {
  return (
    <section className="relative overflow-hidden bg-slate-950">
      <div
        className="pointer-events-none absolute left-[58%] top-[-220px] -z-0 h-[620px] w-[900px] -translate-x-1/2 rounded-full bg-blue-600/20 blur-[120px]"
        aria-hidden="true"
      />

      <div
        className="pointer-events-none absolute inset-x-0 top-0 -z-0 h-[520px] bg-[radial-gradient(circle_at_70%_25%,rgba(14,165,233,0.09),transparent_42%)]"
        aria-hidden="true"
      />

      <div className="relative mx-auto grid max-w-7xl gap-10 px-4 pb-16 pt-10 sm:px-6 sm:pb-20 sm:pt-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-10 lg:px-8 lg:pb-24 lg:pt-16">
        <div>
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-slate-300 sm:text-lg">
            Real-time ride management, dispatch and driver tracking —
            connected across guest, driver and admin portals from booking to
            drop-off.
          </p>
          ...
          <div className="mt-10 grid max-w-xl grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2 sm:gap-y-3.5">
            {HIGHLIGHTS.map((item) => (
              <div
                key={item}
                className="flex items-start gap-2.5 text-[12px] font-semibold leading-5 text-slate-300 sm:text-[13px]"
              >
